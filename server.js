...
app.get("/", function(req, res) {
    res.sendFile(path.join(__dirname, 'views/introduction.html')); //sendFile transfers the filefile at the given path with the response, in this case the Home Page.
});

//when the server gets a request for "/structure"
app.get("/structure", function(req, res) {
    res.sendFile(path.join(__dirname, 'views/structure.html')); //respond sending the routes page 
});

//when the server gets a request for "/heroku"
app.get("/heroku", function(req, res) {
    res.sendFile(path.join(__dirname, 'views/MySQLHerokuDeploymentProcess.html')); //respond sending the routes page 
});

//when the server gets a request for "/orm"
app.get("/orm", function(req, res) {
    res.sendFile(path.join(__dirname, 'views/orm.html')); //respond sending the routes page 
});
